...
import me.sonar.sdkmanager.model.db.{AppMetadataDao, AppMetadata}
        ...
        future {
            platform match {
                case `android` =>
                    ...
                    appMetadataDao.save(AppMetadata(platform.name() + "-" + id, platform.name(), pairsHash.getOrElse("category:", "")))
                case _ =>
            }
        }
        None
