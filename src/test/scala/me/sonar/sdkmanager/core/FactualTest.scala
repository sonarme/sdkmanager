...
class FactualTest extends SpringComponentTest {
    ...
    "factual" should "return places data" in {
        val factualRequest = new FactualRequest(query = Option("sushi"))
        ...
        val request2 = new FactualRequest(query = Option("sushi"), geo = Option(FactualGeo(40.745396, -73.983661, 1000)))
        ...
        val request3 = new FactualRequest(query = Option("burger"), filter = Option(FactualFilter(region = Option(List("MA", "NH")))))
